...
自動で取り込まれ、.poファイルが作成されます。.poファイルには以下が自動で挿入されます:

- **翻訳メモリ（TMX）からの訳文**: 過去の確定翻訳を再利用
- **機械翻訳（DeepL API）**: [tsuji](https://github.com/doc-l10n-kit/tsuji) ツールを使用した自動翻訳

これらの訳文は翻訳の際の下訳としてご活用下さい。

但し、機械翻訳による訳文はてにおは等が不自然な部分も多く、「要確認」（fuzzy）としてマークされており、「要確認」マークを外さない限り、翻訳として反映されません。
訳文をレビュー、修正し、「要確認」マークを外して下さい。

**自動化ワークフロー:**
- 毎週日曜日: upstream 同期、PO 抽出、翻訳メモリ適用、機械翻訳
- 毎日: 翻訳メモリ更新、統計更新、サイトビルド・デプロイ
...
upstream側で元ファイルが更新された場合、GitHub Actionsのワークフローにより自動的にIssueが作成され通知されます。[l10n/stats/override.csv](l10n/stats/override.csv) で、どのファイルが更新されたかを確認できます。
